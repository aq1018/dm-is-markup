  ...
  module Is
    module Markup
      ...
      def is_markup(options)
        ...
        options = {:markup_property => :filtered_content, :filter_property => :filter_type }.merge(options)
        ...
        source_property = properties.detect{|p| p.name == options[:source].to_sym && p.type == Text}
        ...
        unless existing_markup_property || existing_filter_property
          property markup_property, Text
          property filter_property, Enum[:textile, :markdown, :wikitext], :nullable => false, :default => :textile
        else
